...
namespace vis
{
	...
	void Visualisation::update_selection_cursor(glm::vec2 mouse_offset, glm::mat4 modelview, float aspect_ratio)
	{
		...
		_cursor_position += 1.f/aspect_ratio * glm::normalize(glm::vec2{x_mapped}) * mouse_offset.x * cursor_speed + glm::normalize(glm::vec2{y_mapped}) * mouse_offset.y * cursor_speed;
